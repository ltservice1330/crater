...
type Config struct {
	...
	// Verification contains configuration for email and SMS verification services.
	// Optional: If both services are disabled, verification codes will only be logged.
	Verification struct {
		// Email contains configuration for email verification service.
		// Optional: If Enable is false, email verification will be disabled.
		Email struct {
			// Enable toggles email verification functionality.
			// Optional: Defaults to false if not specified.
			Enable bool `json:"enable"`

			// Provider specifies the email service provider (sendgrid, mailgun, ses).
			// Required if Enable is true: Must be one of the supported providers.
			Provider string `json:"provider"`

			// APIKey is the API key for the email service provider.
			// Required if Enable is true: Must be a valid API key.
			APIKey string `json:"apiKey"`

			// FromEmail is the sender email address for verification emails.
			// Required if Enable is true: Must be a valid email address.
			FromEmail string `json:"fromEmail"`

			// FromName is the sender name for verification emails.
			// Optional: Defaults to "System" if not specified.
			FromName string `json:"fromName"`
		} `json:"email"`

		// SMS contains configuration for SMS verification service.
		// Optional: If Enable is false, SMS verification will be disabled.
		SMS struct {
			// Enable toggles SMS verification functionality.
			// Optional: Defaults to false if not specified.
			Enable bool `json:"enable"`

			// Provider specifies the SMS service provider (twilio, alibaba, custom).
			// Required if Enable is true: Must be one of the supported providers.
			Provider string `json:"provider"`

			// AccessKeyID is the access key ID for Alibaba Cloud SMS.
			// Required if Enable is true and Provider is "alibaba".
			AccessKeyID string `json:"accessKeyId"`

			// AccessKeySecret is the access key secret for Alibaba Cloud SMS.
			// Required if Enable is true and Provider is "alibaba".
			AccessKeySecret string `json:"accessKeySecret"`

			// SignName is the SMS signature name for Alibaba Cloud.
			// Required if Enable is true and Provider is "alibaba".
			SignName string `json:"signName"`

			// TemplateCode is the SMS template code for Alibaba Cloud.
			// Required if Enable is true and Provider is "alibaba".
			TemplateCode string `json:"templateCode"`

			// RegionID is the region ID for Alibaba Cloud (e.g., cn-hangzhou).
			// Optional: Defaults to "cn-hangzhou" if not specified.
			RegionID string `json:"regionId"`

			// AppID for the custom SMS provider.
			AppID string `json:"appId"`

			// Secret for the custom SMS provider.
			Secret string `json:"secret"`

			// APIURL is the base URL for the custom SMS provider.
			APIURL string `json:"apiUrl"`

			// BusiCode for the custom SMS provider.
			BusiCode string `json:"busiCode"`

			// VisualMobile for the custom SMS provider.
			VisualMobile string `json:"visualMobile"`
		} `json:"sms"`
	} `json:"verification"`
